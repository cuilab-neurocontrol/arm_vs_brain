...
# -*- coding: utf-8 -*-
"""
PD analysis aligned to GO cue for Monkey Bohr Manual Control
...
for raw_dirname in tqdm(os.listdir(data_folder)):
    ...
    if len(success_trials) == 0:
        continue
    mo_times = success_trials['go_cue_on_time']
...
if len(all_df) > 0:
    ...
    df_combined.to_csv(f'{base_path}/GO_pd.csv', index=False)
    print(f"Results saved to: {base_path}/GO_pd.csv")
