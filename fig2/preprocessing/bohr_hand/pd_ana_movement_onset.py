...
# -*- coding: utf-8 -*-
"""
PD analysis aligned to movement onset for Monkey Bohr Manual Control

This script calculates preferred direction (PD) metrics for each unit
at each time bin relative to movement onset.
...
data_folder = os.path.join(SUBMISSION_ROOT, 'nwb_kilosort', 'bohr', 'Data_recording')
base_path = os.path.join(SUBMISSION_ROOT, 'fig2', 'preprocessing', 'bohr_hand')
...
for raw_dirname in tqdm(os.listdir(data_folder)):
    ...
    # Alignment time: movement onset (with -0.108s offset to match reference)
    mo_times = success_trials['movement_onset_time'] - 0.1
