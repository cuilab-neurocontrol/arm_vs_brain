...
# -*- coding: utf-8 -*-
"""
PD analysis aligned to movement onset for Monkey Leb Feedback (2DBC)

This script calculates preferred direction (PD) metrics for each unit
at each time bin relative to movement onset (position norm > 1.5).
...
def calculate_movement_onset_time(pos_segment):
    """Calculate movement onset time when position norm exceeds threshold"""
    norms = np.linalg.norm(pos_segment.to_numpy(), axis=1)
    mo_indices = np.where(norms > 1.5)[0]
    if len(mo_indices) > 0:
        return float(pos_segment.t[mo_indices[0]])
    return None
...
base_path = os.path.join(SUBMISSION_ROOT, 'fig2', 'preprocessing', 'leb_feedback')
...
for raw_dirname in tqdm(os.listdir(data_folder)):
    ...
    trial_mo = [calculate_movement_onset_time(i) if i is not None else None for i in trial_pos]
...
if len(all_df) > 0:
    ...
    output_path = f'{base_path}/movement_onset_pd.csv'
