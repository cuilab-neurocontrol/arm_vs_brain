...
# -*- coding: utf-8 -*-
"""
PD analysis aligned to target on (marker 1) for Monkey Leb Feedback (2DBC)

This script calculates preferred direction (PD) metrics for each unit
at each time bin relative to target appearance.
...
for raw_dirname in tqdm(os.listdir(data_folder)):
    ...
    # Alignment time: target on (marker 1)
    mo_times = np.array([i.t[np.where(i.d[:] == 1)[0][0]] for i in df['markers'].values])
...
if len(all_df) > 0:
    ...
    output_path = f'{base_path}/target_on_pd.csv'
