...
# -*- coding: utf-8 -*-
"""
PD analysis aligned to GO signal (marker 3) for Monkey Leb Feedback (2DBC)

This script calculates preferred direction (PD) metrics for each unit
at each time bin relative to GO signal.
...
for raw_dirname in tqdm(os.listdir(data_folder)):
    ...
    # Alignment time: GO signal (marker 3)
    mo_times = np.array([i.t[np.where(i.d[:] == 3)[0][0]] for i in df['markers'].values])
...
if len(all_df) > 0:
    ...
    output_path = f'{base_path}/GO_pd.csv'
