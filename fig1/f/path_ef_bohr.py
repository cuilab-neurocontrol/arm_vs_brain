...
# -*- coding: utf-8 -*-
"""
Path Efficiency analysis for Monkey Bohr (Brain Control)

This script calculates the path efficiency for successful trials where
training flag is 0. Path efficiency = straight-line distance / actual trajectory length.
...
def calculate_trajectory_length(points):
    """Calculate total length of trajectory from a series of points"""
    total_length = 0.0
    for i in range(len(points) - 1):
        total_length += euclidean_distance(points[i], points[i + 1])
    return total_length
...
for raw_dirname in os.listdir(data_folder):
    ...
    # Get position data for successful trials (marker 20 present) with training flag = 0
    select_pos = [i for i, j, z in zip(trial_pos, select_trial, trial) if (j == 1) and (20 in z)]

    # Calculate path efficiency: straight-line distance / actual trajectory length
    path_eff = [np.linalg.norm(i[-1]) / calculate_trajectory_length(i) for i in select_pos]

    pf_dict['group_name'].append('B')
    pf_dict['group_value'].append(np.mean(path_eff))

# Save results
data = pd.DataFrame(pf_dict)
output_path = os.path.join(output_folder, 'path_ef_bohr.csv')
