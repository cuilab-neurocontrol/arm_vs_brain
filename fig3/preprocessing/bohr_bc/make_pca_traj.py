...
# -*- coding: utf-8 -*-
"""
PCA Trajectory Preprocessing for Fig3 - Bohr BC (Semi2DBC120)
"""
...
def process_dataset(data_path, output_path):
    ...
    try:
        ...
        # No need to filter kilosort - already filtered in nwb_kilosort
...
# Main
nwb_base = os.path.join(SUBMISSION_ROOT, 'nwb_kilosort', 'bohr', 'Brain_control')
output_base = os.path.join(SUBMISSION_ROOT, 'fig3', 'preprocessing', 'bohr_bc')

# Get Semi2DBC120 datasets
datasets = [d for d in os.listdir(nwb_base) if 'Semi2DBC120' in d]
print(f"Found {len(datasets)} Semi2DBC120 datasets")
