...
# -*- coding: utf-8 -*-
"""
VMPS Timeseries Analysis for Fig3e
Calculates mean vmps (exec variance in prep space) vs exec_t (with perp_t fixed at -0.3)
"""
...
def process_condition(merged_file, output_dir, condition_name):
    ...
    # Aggregate by exec_t - using vmps instead of alignment_index
    stats_df = filtered_df_.groupby('exec_t')['vmps'].agg(['mean', 'count', 'sem']).reset_index()
    ...
    output_csv = f'{output_dir}/vmps_timeseries_{condition_name}.csv'
    ...
    print(f"    y range: [{final_stats_df['y'].min():.2f}%, {final_stats_df['y'].max():.2f}%]")
